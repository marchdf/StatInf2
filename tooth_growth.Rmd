...
```{r boxplot_both}
ggplot(data=df, aes(x=delivery,y=len)) +
    geom_boxplot(aes(fill=delivery)) + facet_wrap(~ delivery) +
    labs(x = "delivery",
         y="Tooth length",
         title="Boxplot of tooth length versus delivery method");
```
...
The null hypothesis is that the delivery method does not affect the tooth growth. The alternative then is that the delivery method does affect the tooth growth. We perform hypothesis testing to see if we can reject the null hypothesis. 
